...
def _hit(page: int, text: str, score: float = 1.0) -> RetrievedPassage:
...
def test_parse_citations_keeps_only_retrieved_pages() -> None:
    # First line of defence against hallucinated page numbers.
    passages = [_hit(1, "a", score=0.9), _hit(2, "b", score=0.5)]
    text = "Something [page 1] and invented [page 42] and real [page 2]."
...
def test_parse_citations_preserves_order_and_duplicates() -> None:
    passages = [_hit(3, "x")]
    citations = parse_citations("see [page 3], also [page 3] again", passages)
    assert [c.page_number for c in citations] == [3, 3]


def test_parse_citations_is_case_insensitive() -> None:
    passages = [_hit(7, "y")]
    citations = parse_citations("see [Page 7] and [PAGE 7]", passages)
...
def test_stub_answerer_cites_top_passage() -> None:
    ...
    answer = stub.answer(
        "ignored",
        [_hit(4, "Nougat parses academic PDFs.", score=0.8), _hit(1, "unrelated")],
    )
    assert isinstance(answer, Answer)
    assert "[page 4]" in answer.text
